...
import { CryptoCurrency, Operation, OperationType } from "../../types";
...
export const getEstimatedFees = (tx: EvmTransaction): BigNumber => {
  if (!tx.type?.eq(2)) {
    return (tx as EvmTransactionLegacy).gasPrice.multipliedBy(tx.gasLimit);
  }
  return (tx as EvmTransactionEIP1559).maxFeePerGas
    .plus((tx as EvmTransactionEIP1559).maxPriorityFeePerGas)
    .multipliedBy(tx.gasLimit);
};
